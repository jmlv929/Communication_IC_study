...
StateInfoType gStateInfoTable[STATENUM] = { // 记录(3,1,7)的全部状态，方便译码
 ...
 void conv_decode_tibi(
    ...
    half_state_num = STATENUM / 2; //给分支测量分配存储空间，硬件则为RAM实现
    memset((void *)metric, MINVALUE, sizeof(Sint32)*STATENUM*2);
    //对每个输入信息bit进行64状态的加比选计算 
    for (k = 0; k < info_len; k++) {
      for (j = 0; j < STATENUM; ) { //总计有32个蝶形单元，所以每次加2
        i = (j+1) / 2;
        tmp_out0 = gStateInfoTable[j].state_out0;
        gamma = 0;
        for (l = 0; l < CODERATE; l++) { //对3个子码依次做分支度量累加，并做选择
    ...
    for (k = info_len - 1; k > -1; k--) {
        if (current_state < half_state_num) { //判断输出值是否小于32
            output_ptr[k] = 0;  //译码结果为0,并更新状态
            last_state = path[current_state+k*STATENUM]; }
        else {
            output_ptr[k] = 1; //译码结果为1,并更新状态
             last_state = path[current_state+k*STATENUM]; }
