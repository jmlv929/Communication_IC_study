...
// Modified:
//   06/13/09  (C-0906-SP1)
//     Removed  error with addr signal
...
module DW_fp_recip (
  ...
  wire [`f_sig_width:0] mz;
  wire [`f_sig_width:0] div_out;
  ...
  wire [2:0] lzd_inb;
  wire [1:0] lzd_inb_post;
  wire rshift_ovfl;
  wire [2 * `f_sig_width + 1 : 0] rshift_out;
  wire sig_below_zero;
  ...
  // NR divider variables definition
  ...
  wire [`f_sig_width:0] quo_in;
  wire [`f_sig_width:0] quo1_in;
  ...
  assign rndi = (faithful_round) ? 3'b1 : rnd;
  assign b = {1'b0, 1'b0, {(exp_width - 1){1'b1}}, {(sig_width){1'b0}}};

  // Unpack the FP Numbers
  assign {signa, ea, sa} = b; // dividend 1
  assign {signb, eb, sb} = a; // divisor
  ...
  assign ea_zero = (ea == 0);
  assign eb_zero = (eb == 0);
  assign ea_inf = (ea == ((({exp_width{1'b1}}>>1) << 1) + {{exp_width-1{1'b0}},1'b1}));
  assign eb_inf = (eb == ((({exp_width{1'b1}}>>1) << 1) + {{exp_width-1{1'b0}},1'b1}));
  ...
  assign sig_nan_result = (ieee_compliance) ? 
                            {{(sig_width - 1){1'b0}}, 1'b1} : 
                            {(sig_width){1'b0}};

  // Exponent Calculation
  generate
    if (ieee_compliance == 1) begin : GEN_ic_eq_1_r2
      assign ez = ($signed({1'b0, ea}) - $signed({1'b0, eb}) 
                  + $signed({1'b0, lzd_inb_post}) - $signed({1'b0, denorm_b}) 
                  + $signed({1'b0, {(exp_width - 1){1'b1}}}));
    end
    else begin : GEN_ic_ne_1_r2
      assign ez = ($signed({1'b0, ea}) - $signed({1'b0, eb}) 
                  + $signed({1'b0, {(exp_width - 1){1'b1}}}));
    end
  endgenerate
  ...
  DW_lzd #(3) U2 (
    .a({1'b0, sb[sig_width - 1:sig_width - 2]}), 
    .dec(dummy), 
    .enc(lzd_inb)
  ); 

  assign lzd_inb_post = (denorm_b) ? lzd_inb[1:0] : 2'b0;

  assign normed_ma = ma;
  assign normed_mb = (ieee_compliance & denorm_b) ? mb << lzd_inb[1:0] : mb;

  assign mb_zero = (ieee_compliance) ? 
                            //(normed_mb[sig_width - 1:0] == 0) & (rshift_amount == 0):
                            (normed_mb[sig_width - 1:0] == 0) :
                            sb_zero;

  //////////////////////////////////////////////////////
  // Integer Reciprocal by Newton-Raphson Method
  //////////////////////////////////////////////////////
  generate
    if (ieee_compliance == 1) begin : GEN_ic1
      assign sig_pre = normed_mb;
    end
    else begin : GEN_icne1
      assign sig_pre = {1'b1, sb[sig_width - 1:0]};
    end
  endgenerate
  ...
  // ROM Lookup Table
  `define addr_sig_lsb ((`f_sig_width >= 9) ? `f_sig_width - 9 : 0)
  `define addr_zero_width ((`f_sig_width >= 9) ? 1 : 9 - `f_sig_width)

  generate 
    if (`f_sig_width >= 9) begin : GEN_sw_ge_9
      assign addr = sig[`f_sig_width - 1:`addr_sig_lsb];
    end
    else begin : GEN_sw_lt_9
      assign addr = {sig[`f_sig_width - 1:0], {(`addr_zero_width){1'b0}}};
    end
  endgenerate
  ...
  // x1 = x0 x (1 + de) = x0 + x0de
  assign x0de = x0 * de; // 9b x (f+2)b = (f+11)b
  ...
  assign x1 = {x0, {(`f_sig_width - 5){1'b0}}} + x0de[`f_sig_width + 10:14];
    end
  endgenerate
  ...
  assign de2 = (`f_sig_width >= 11) ?
                 de[`de2_range_1:`de2_range_2] * de[`de2_range_1:`de2_range_2] :
                 0;
  assign de2h = (`f_sig_width >= 11) ?
                de2[`de2h_range_1:`de2h_range_2] : 0; // (f-10)b
  assign de2_g = de * de;

  // x1de2 = x1 * de2, [(f-10)b x (f-10)b]
  assign x1de2 = (`f_sig_width >= 11) ? 
                 x1[`x1de2_range_1:`x1de2_range_2] * de2h : 0;
  assign x1de2_g = x1 * de2_g;

  // x2 Result
  assign x2 = x1 + x1de2[`de2h_range_1:`de2h_range_2];
  ...
  assign de2h_t = (`f_sig_width >= 25) ? de2h[`de2h_t_range_1:`de2h_t_range_2] : 0;
  assign de4 = de2h_t * de2h_t;

  // x2de4 [(f-23)b x (f-23)b]
  assign x2de4 = (`f_sig_width >= 25) ?
                   x2[`x2de4_range_1:`x2de4_range_2] * de4[`x2de4_range_3:`x2de4_range_4] : 0;
  assign x2de4_t = (`f_sig_width >= 25) ? 
                     x2de4[`x2de4_t_range_1:`x2de4_t_range_2] : 0; // [(f-24)b]
  assign x3 = x2 + x2de4_t;

  //--------------------------------------
  // RNU Logic
  //--------------------------------------
  generate
    if (`f_sig_width == 8) begin : GEN_sw8
      assign x0_inc = x0 + 1;
    end
    else if (`f_sig_width < 8) begin : GEN_swlt8
      assign x0_inc = x0 + {1'b1, {(`x0_rbit + 1){1'b0}}};
    end
    else begin : GEN_swgt8
      assign x0_inc = 9'b0;
    end
  endgenerate
  ...
  generate
    if (`f_sig_width == 8) begin : GEN_sweq8
      //assign x0_rounded = x0[8:`x0_rbit + 1];
      assign x0_rounded = x0_inc[8:`x0_rbit + 1];
    end
    else begin : GEN_swne8
      assign x0_rounded = (x0[`x0_rbit]) ? x0_inc[8:`x0_rbit + 1] :
                                           x0[8:`x0_rbit + 1];
    end
  endgenerate
  ...
  `define quo_x0_lsb (((faithful_round == 0) & (`f_sig_width <= 8))  ? \
                       8 - `f_sig_width : 0)

  generate 
    if (faithful_round == 1) begin : GEN_fr_eq_1
      if (`f_sig_width <= 8) begin : GEN_fr1_sw_z0
        assign quo = x0_rounded;
      end
      else if (`f_sig_width <= 14) begin : GEN_fr1_sw_z1
        assign quo = x1_rounded;
      end
      else if (`f_sig_width <= 30) begin : GEN_fr1_sw_z2
        assign quo = x2_rounded;
      end
      else begin : GEN_fr1_sw_z3
        assign quo = x3_rounded;
      end
    end
    else begin : GEN_fr_ne_1
      if (`f_sig_width <= 8) begin : GEN_fr0_z0
        assign quo = x0[8:`quo_x0_lsb];
      end
      else if (`f_sig_width <= 14) begin : GEN_fr0_z1
        assign quo = x1[`f_sig_width + 3:3];
      end
      else if (`f_sig_width <= 30) begin : GEN_fr0_z2
        assign quo = x2[`f_sig_width + 3:3];
      end
      else begin : GEN_fr0_z3
        assign quo = x3[`f_sig_width + 3:3];
      end
    end
  endgenerate

  assign rem = {{(sig_width){1'b0}}, 1'b1};
  ///////////////////////
  // For the Test
  ///////////////////////
  ...
  assign quo2 = quo + 2;
  assign quo3 = quo + 3;
  assign qd1 = quo1 * sig; // (f+2) x (f+2)
  assign qd_m = qd1[`f_sig_width + 2];

  // logics for ieee_compliance = 1 and faithful_round = 0
  assign quo_in = quo;
  assign quo1_in = (sig_zero) ? {1'b1, {(`f_sig_width){1'b0}}} : quo1;
  assign quo_rshout = quo_in >> rshift_amount;
  assign quo1_rshout = quo1_in >> rshift_amount;
  ...
  assign quo_rsh_final = (sig_zero) ? quo1_rsh_rounded :
                         (qd_m) ? quo1_rsh_rounded : quo_rsh_rounded;
  ...
  assign shift_req = ~(normed_mb[sig_width - 1:0] == 0);
  assign mz = (faithful_round) ? quo : quo[`f_sig_width:1]; // f+1 bits
  ...
  assign rshift_ovfl = (`f_sig_width + ez_norm + 1 < 0); // -ez > f + 1
  ...
  generate
    if (ieee_compliance == 1) begin : GEN_ie1
      assign rshift_out = {mz, {(`f_sig_width + 1){1'b0}}} >> rshift_amount;
    end
    else begin : GEN_iene1
      assign rshift_out = {(2 * `f_sig_width + 2){1'b0}};
    end
  endgenerate
  ...
  // Final Division Out
  generate
    if (ieee_compliance == 1) begin : GEN_ie1_a
      assign div_out = rshift_out[2 * `f_sig_width + 1:`f_sig_width + 1];
    end
    else begin : GEN_iene1_a
      assign div_out = mz;
    end
  endgenerate
  ...
  assign guard_bit0 = (faithful_round) ? 1'b0 :
                      (sig_zero) ? 1'b0 :
                      (ieee_compliance) ? quo_rshout[1] : mz_guard_bit0;
  assign round_bit0 = (faithful_round) ? 1'b0 :
                      (sig_zero) ? 1'b0 :
                      (ieee_compliance) ? quo_rshout[0] : mz_round_bit0;
  assign sticky_bit0 = (faithful_round) ? 1'b0 :
                       (sig_zero) ? 1'b0 : 1'b1;
  assign guard_bit1 = (faithful_round) ? 1'b0 :
                      (sig_zero) ? 1'b0 :
                      (ieee_compliance) ? quo1_rshout[1] : mz_guard_bit1;
  assign round_bit1 = (faithful_round) ? 1'b0 :
                      (sig_zero) ? 1'b0 :
                      (ieee_compliance) ? quo1_rshout[0] : mz_round_bit1;
  assign sticky_bit1 = (faithful_round) ? 1'b0 :
                       (sig_zero) ? 1'b0 : 1'b1;
  ...
  assign quo_sel0 = (inc0) ? quo2 : quo;
  assign quo_sel1 = (inc1) ? quo3 : quo1;

  assign quo_final = (qd_m) ? quo_sel1 : quo_sel0;

  // Rounding Addition
  assign mz_rounded_pre = (mb_zero) ? 
                            {1'b0, rshift_amount_one, {(sig_width - 1){1'b0}}} :
                          (faithful_round) ? div_out : quo_final;
  assign mz_rounded = (faithful_round) ? mz_rounded_pre : 
                      (ieee_compliance) ? quo_rsh_final :
                                          mz_rounded_pre[`f_sig_width:1];

  // Huge, Tiny Setup
  assign over_inf = (ez_norm[exp_width:0] >= ((({exp_width{1'b1}}>>1) << 1) + {{exp_width-1{1'b0}},1'b1})) & ~ez_norm[exp_width + 1];
  ...
  // suggestion for fix.  5/17/10
  assign infinity = (faithful_round) ? 
                       over_inf :
                       over_inf & RND_eval0[`RND_HugeInfinity];
  ...
  assign ez_norm_modified = (ieee_compliance & (ez_norm == 0) & mz_rounded[sig_width]) ? 
                              {{(exp_width + 1){1'b0}}, 1'b1} : ez_norm;

  // Status Flag Setup
  assign status[7] = zero_b;
  assign status[6] = 1'b0;
  assign status[5] = normal_case & (over_inf | sig_below_zero | ~mb_zero);
  assign status[4] = normal_case & over_inf;
  assign status[3] = normal_case & below_zero;
  ...
  always @(guard_bit0 or round_bit0 or sticky_bit0 or sign or rndi) begin
  ...
  always @(guard_bit1 or round_bit1 or sticky_bit1 or sign or rndi) begin
