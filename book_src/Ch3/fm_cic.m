...
B = 12;                                     %% Number of bits to represent fixed point filter coefficients 
Fs = 80e6;                                  %% (High) Sampling frequency in Hz (before decimation) 
Fc = (Fs/R)*0.3;                            %% Passband edge in Hz 
%% %%%%% fir2.m parameters %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
legend('CIC','CIC Comp','Total Response (浮点)','Total Response (定点)') 
ylim([-100 5]); 
...
%% design my CIC contrast figure
figure; 
plot(wt, Mcic, wt, Mciccomp, wt, Mt,wt, Mzt); 
legend('CIC','CIC补偿','浮点频响','定点频响') 
ylim([-100 5]); 
xlim([0,Fs/R*2]);
title('Frequency Sampling Method'); 
grid 
